package me.shockpast.roflan.constants;

import net.kyori.adventure.text.format.TextColor;

public class Colors {
    public static final TextColor Blue = TextColor.color(66, 135, 245);
    public static final TextColor Red = TextColor.color(240, 55, 55);
    public static final TextColor White = TextColor.color(255, 255, 255);
    public static final TextColor Gray = TextColor.color(170, 170, 170);
    public static final TextColor DarkGray = TextColor.color(85, 85, 85);
    public static final TextColor Yellow = TextColor.color(239, 245, 66);
    public static final TextColor Green = TextColor.color(72, 245, 66);
}
